...
class BigComplex {
  ...
  multiply(other) {
    return new BigComplex(
      ...
      this.real.times(other.imag).plus(this.imag.times(other.real))
    );
  }
  ...
  normSquared() {
    return this.real.times(this.real).plus(this.imag.times(this.imag));
  }
}

function toBig(value, fallback = '0') {
  try {
...
function mandelbrotIterationsBig(c, maxIter) {
  const escapeLimit = new Big(4);
  let z = new BigComplex(new Big(0), new Big(0));

  for (let i = 0; i < maxIter; i++) {
    if (z.normSquared().gt(escapeLimit)) return i;
    z = z.multiply(z).add(c);
  }
...
function juliaIterationsBig(z, c, maxIter) {
  const escapeLimit = new Big(4);
...
function generateMandelbrotBig(canvas, params) {
  const { width, height, maxIter, colorScheme } = params;
  const zoom = toBig(params.zoom, '1');
  const centerX = toBig(params.centerX, '0');
  const centerY = toBig(params.centerY, '0');

  if (zoom.eq(0)) {
    throw new Error('Zoom cannot be zero in Big Number mode.');
  }
  ...
  const minDimension = new Big(Math.min(width, height));
  const scale = new Big(4).div(zoom.times(minDimension));
  const halfWidth = new Big(width).div(2);
  const halfHeight = new Big(height).div(2);

  for (let y = 0; y < height; y++) {
    const yOffset = new Big(y).minus(halfHeight).times(scale);

    for (let x = 0; x < width; x++) {
      const xOffset = new Big(x).minus(halfWidth).times(scale);
      const c = new BigComplex(centerX.plus(xOffset), centerY.plus(yOffset));

      const iterations = mandelbrotIterationsBig(c, maxIter);
...
function generateJuliaBig(canvas, params) {
  const { width, height, maxIter, colorScheme } = params;
  const zoom = toBig(params.zoom, '1');
  const centerX = toBig(params.centerX, '0');
  const centerY = toBig(params.centerY, '0');
  const juliaCReal = toBig(params.juliaCReal, '0');
  const juliaCImag = toBig(params.juliaCImag, '0');
  ...
  const halfWidth = new Big(width).div(2);
  const halfHeight = new Big(height).div(2);
  const c = new BigComplex(juliaCReal, juliaCImag);
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < width; x++) {
      const xOffset = new Big(x).minus(halfWidth).times(scale);
      const z = new BigComplex(centerX.plus(xOffset), centerY.plus(yOffset));

      const iterations = juliaIterationsBig(z, c, maxIter);
      const color = colorFromIterations(iterations, maxIter, colorScheme);
...
export function generateMandelbrot(canvas, params) {
  if (params.precisionMode === 'big') {
    generateMandelbrotBig(canvas, params);
    return;
  }
...
export function generateJulia(canvas, params) {
  if (params.precisionMode === 'big') {
    generateJuliaBig(canvas, params);
    return;
  }
